...
ConeShell::ConeShell(int num_cols, int num_rows,
...
std::vector<GLfloat> ConeShell::getPointData(int col, int row) {
    ...
    normal = getTransformNormals() *
            glm::vec4(cos( (static_cast<float>(col)/getNumCols()) * 2 * 3.1415),
                           sin( (static_cast<float>(col)/getNumCols()) * 2 * 3.1415 ),
                      -2/sqrt(5), 1.0f);
