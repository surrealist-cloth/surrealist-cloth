...
void GLVertWidget::paintGLTransformationsVertexShaders() {
    // Clear the color and depth buffers.
        ...
        glm::mat4 model(1.f);
        model = glm::translate(model, glm::vec3(0, 0.5 + fabs(sin(3 * time)), 0));
...
//        model = glm::translate(model, glm::vec3(0, 0.5, 0));
        ...
        glUniform3f(glGetUniformLocation(m_program, "color"), 0.1, 0.8, 0.4);

        // TODO: Scale the square x2. (Task 7)
        model = glm::scale(glm::mat4(1.f), glm::vec3(2, 2, 2));


        // TODO: Rotate the square to lie flat on the XZ plane. (Task 7)
        model = glm::rotate(model, glm::pi<float>() / 2.f, glm::vec3(1, 0, 0));
        glUniformMatrix4fv(glGetUniformLocation(m_program, "model"), 1, GL_FALSE, glm::value_ptr(model));
